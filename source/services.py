...
COMPANY_DOWNLOAD_URL = 'https://query1.finance.yahoo.com/v7/finance/download/{company}?period1={start}&period2={' \
                       'end}&interval={interval}&events=history&includeAdjustedClose=true '


def save_company__to_db(company, start_date: int = 0, end_date: int = int(time.time()), interval: str = '1d'):
    df = pd.read_csv(COMPANY_DOWNLOAD_URL.format(
...
def fetch_data_for_company(max_records: int):
    finances = _read_db()
    by_company = finances[finances['company'] == 'DOCU'].head(max_records)
